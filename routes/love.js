...
router.get('/woman',function(req,res,next) {
  ...
  var sql = `select count(*) as namesCount from ilwoman`;
...
router.get('/woman/:useridx',function(req,res,next) {
  ...
  var sql = `SELECT * FROM ilwoman , iuser where ilwoman.iuser_idx = ? AND iuser.iuser_idx = ?; `
      ...
      if(err){
        ...
        conn.query(sql,params,function(err,rows,field) {
          if(rows[0].iuser_gender != "여자"){
            ...
            sql = `INSERT INTO ilwoman 
            (iuser_idx,ilwoman_comment) 
            values(?,?)`
...
router.get('/man',function(req,res,next) {
  var sess = req.session;
  var sql = `select count(*) as namesCount from ilman`;
  conn.query(sql,function(err,rows,field) {
    if(err){
      console.log(err);
      res.render("love/man",{
...
router.get('/man/:useridx',function(req,res,next) {
  var useridx = req.params.useridx;
  var sql = `SELECT * FROM ilman , iuser where ilman.iuser_idx = ? AND iuser.iuser_idx = ?; `
  var params = [useridx,useridx];
